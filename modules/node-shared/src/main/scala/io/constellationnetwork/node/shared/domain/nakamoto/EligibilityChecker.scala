...
/** VRF-based leader eligibility checker using LDD snowplow threshold.
  ...
  *   1. Computes VRF proof over (eta || slot) 2. Extracts VRF output hash (64 bytes) 3. Normalizes to [0,1) as testValue 4. Computes
  *      threshold from LDD snowplow + relative stake 5. Eligible if threshold > testValue
  */
object EligibilityChecker {

  private val vrf = new EcVrf25519()

  /** Compute LDD snowplow threshold. threshold = 1 - (1 - f(δ))^relativeStake
    */
  ...
  /** Compute VRF proof for a slot. Message = eta (32 bytes) || slot (8 bytes big-endian)
    */
  ...
  /** Normalize VRF output to [0, 1) for threshold comparison. testValue = BigInt(vrfOutput) / 2^512
    */
  ...
  /** Check if a validator is eligible to produce a snapshot in the given slot.
    *
    * @param vrfSK
    *   32-byte VRF secret key (Ed25519 seed)
    * @param slot
    *   current slot
    * @param slotGap
    *   slots since last snapshot was produced by anyone
    * @param eta
    *   current epoch randomness (32 bytes)
    * @param relativeStake
    *   this validator's stake fraction [0,1]
    * @param config
    *   LDD parameters
    * @return
    *   Some(proof, vrfOutput) if eligible, None if not
    */
  ...
  /** Compute epoch randomness from previous eta and VRF outputs. eta_{e+1} = Blake2b-256(eta_e || epoch || concat(rhoNonceHashes))
    */
